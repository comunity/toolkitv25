...
> **Tip**: When viewing logs, start by filtering for errors to identify critical issues, then expand to warnings and information logs for context.
...
> **Important**: When you change the time range, the Available Components dropdown automatically updates to show only components that have logged data within that period. The dropdown displays the count of components found (e.g., “3 components found”).
...
> **Tip**: When the Attributes search toggle is enabled, your search will include these expanded details, allowing you to find logs based on information not visible in the collapsed view.
...
4. **Open the Code Editor** for that entity.
5. **Click the green Code Snippets button** in the top right corner.
...
**Example: Adding an Info Log to OnAdd**

```csharp
// In your entity's OnAdd method:
Tracer.LogInfo("Tag added: " + entity.Name);
```

**Example: Adding an Error Log with Context**

```csharp
// In an error handling block:
Tracer.LogError("Failed to process payment for order: " + orderId);
```

**Example: Adding Custom Attributes**

```csharp
// Using the Log with Attributes snippet:
Tracer.LogInfo("Order processed", new Dictionary<string, object> {
    { "OrderId", orderId },
    { "Amount", totalAmount },
...
> **Note**: This import should be added automatically in newer project templates. If you’re working with an older project, you may need to add it manually.
...
> **Note**: Previously, debugging communication issues required contacting support to investigate database logs. With Platform Logs, you can now self-serve this investigation.
...
> **Note**: These enhancements are dependent on platform component updates and are subject to change.
...
> **Note**: The Logs interface design may be updated based on UX team feedback. Future releases may include improved layout options for component selection and additional filtering capabilities.
